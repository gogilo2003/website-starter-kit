...
const loadMore = () => {
    ...
    router.visit(url, {
        ...
        preserveState: true, // Don't reset page state
        only: ['products'], // Only update the 'products' prop (important!)
        headers: {
            'X-Inertia-Partial-Data': 'products', // Tell backend we're only requesting partial data
            'X-Inertia-Partial-Component': 'Web/Products/Index', // Optional: specify component if needed
        },
        onSuccess: (page) => {
            // Append new products from the partial response
    ...
    <WebLayout :title="title">
        ...
        <div class="bg-white pb-20 pt-16">
            <Container>
                <div
                    v-if="product_intro"
                    class="mb-16 text-center text-lg"
                    v-text="product_intro"
                ></div>
                ...
                <div v-if="loading" class="mt-12 text-center">
                    <div
                        class="inline-block h-8 w-8 animate-spin rounded-full border-b-2 border-gray-900"
                    ></div>
                ...
                <!-- End of list -->
                <div
                    v-if="!hasMore && !loading && productList.length > 0"
